---
summary: "Use MiniMax M2.1 in BonsaiOS"
read_when:
  - You want MiniMax models in BonsaiOS
...
```bash
bonsaios plugins enable minimax-portal-auth  # skip if already loaded.
bonsaios gateway restart  # restart if gateway is already running
bonsaios onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/claimhawk/bonsaios-docs/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `bonsaios.json`:
...
- Use `bonsaios models list` and `bonsaios models set minimax/MiniMax-M2.1` to switch.
...
- Running `bonsaios configure` and selecting **MiniMax M2.1**, or
