---
summary: "Use Venice AI privacy-focused models in BonsaiOS"
read_when:
  - You want privacy-focused inference in BonsaiOS
...
```bash
bonsaios onboard --auth-choice venice-api-key
```
...
```bash
bonsaios chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, BonsaiOS shows all available Venice models. Pick based on your needs:
...
```bash
bonsaios models set venice/claude-opus-45
bonsaios models set venice/llama-3.3-70b
```

List all available models:

```bash
bonsaios models list | grep venice
```

## Configure via `bonsaios configure`

1. Run `bonsaios configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
BonsaiOS automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
bonsaios chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
bonsaios chat --model venice/claude-opus-45

# Use uncensored model
bonsaios chat --model venice/venice-uncensored

# Use vision model with image
bonsaios chat --model venice/qwen3-vl-235b-a22b

# Use coding model
bonsaios chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `bonsaios models list` to see currently available models. Some models may be temporarily offline.
