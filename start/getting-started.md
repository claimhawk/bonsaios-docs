...
Fastest chat: open the Control UI (no channel setup needed). Run `bonsaios dashboard`
and chat in the browser, or open `http://127.0.0.1:18789/` on the gateway host.
Docs: [Dashboard](/web/dashboard) and [Control UI](/web/control-ui).

Recommended path: use the **CLI onboarding wizard** (`bonsaios onboard`). It sets up:
...
- **Recommended:** Brave Search API key for web search. Easiest path:
  `bonsaios configure --section web` (stores `tools.web.search.apiKey`).
...
- OAuth credentials (legacy import): `~/.bonsaios/credentials/oauth.json`
...
If you’re hacking on BonsaiOS itself, run from source:
...
```bash
node bonsaios.mjs gateway --port 18789 --verbose
```
...
If `bonsaios health` shows “no auth configured”, go back to the wizard and set OAuth/key auth — the agent won’t be able to respond without it.

Tip: `bonsaios status --all` is the best pasteable, read-only debug report.
Health probes: `bonsaios health` (or `bonsaios status --deep`) asks the running gateway for a health snapshot.
