---
summary: "First-run onboarding flow for BonsaiOS (macOS app)"
read_when:
...
- Writes credentials to `~/.bonsaios/credentials/oauth.json`
...
The app can install the global `bonsaios` CLI via npm/pnpm so terminal
...
On the first agent run, BonsaiOS bootstraps a workspace (default `~/.bonsaios/workspace`):
...
```bash
bonsaios webhooks gmail setup --account you@gmail.com
```
...
- `~/.bonsaios/credentials/oauth.json`
