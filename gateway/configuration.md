---
summary: "All configuration options for ~/.bonsaios/bonsaios.json with examples"
read_when:
...
BonsaiOS reads an optional **JSON5** config from `~/.bonsaios/bonsaios.json` (comments + trailing commas allowed).

If the file is missing, BonsaiOS uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.bonsaios/workspace`). You usually only need a config to:
...
BonsaiOS only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `bonsaios doctor`, `bonsaios logs`, `bonsaios health`, `bonsaios status`, `bonsaios service`, `bonsaios help`).
- Run `bonsaios doctor` to see the exact issues.
- Run `bonsaios doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `bonsaios config set`. Keep a backup of `~/.bonsaios/bonsaios.json`.
...
```bash
bonsaios gateway call config.get --params '{}' # capture payload.hash
bonsaios gateway call config.apply --params '{
...
```bash
bonsaios gateway call config.get --params '{}' # capture payload.hash
bonsaios gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.bonsaios/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@bonsaios", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.bonsaios/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.bonsaios/workspace" }],
}
```
...
{ "$include": "/etc/bonsaios/base.json5" } // absolute
...
```json5
// ~/.bonsaios/clients/mueller/agents.json5
[
...
```json5
// ~/.bonsaios/clients/mueller/broadcast.json5
{
...
BonsaiOS reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, BonsaiOS runs your login shell and imports only the missing expected keys (never overrides).
...
- `BONSAIOS_LOAD_SHELL_ENV=1`
- `BONSAIOS_SHELL_ENV_TIMEOUT_MS=15000`
...
BonsaiOS stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.bonsaios/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.bonsaios/credentials/oauth.json` (or `$BONSAIOS_STATE_DIR/credentials/oauth.json`)
...
- `~/.bonsaios/agent/*` (migrated by `bonsaios doctor` into `~/.bonsaios/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `BONSAIOS_OAUTH_DIR`
- Agent dir (default agent root override): `BONSAIOS_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, BonsaiOS imports `oauth.json` entries into `auth-profiles.json`.
...
If set, BonsaiOS derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/bonsaios/bonsaios-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/bonsaios/bonsaios.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/bonsaios/bonsaios.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.bonsaios/credentials/whatsapp/biz
          // authDir: "~/.bonsaios/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `bonsaios doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@bonsaios", "bonsaios"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@bonsaios"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.bonsaios/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.bonsaios/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.bonsaios/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.bonsaios/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.bonsaios/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.bonsaios/workspace-home" },
      { id: "work", workspace: "~/.bonsaios/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `bonsaios.json`).
...
BonsaiOS starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["bonsaios-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-bonsaios",
...
BonsaiOS starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `bonsaios plugins install @bonsaios/mattermost` (or `./extensions/mattermost` from a git checkout).
...
BonsaiOS starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
BonsaiOS spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, BonsaiOS
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/bonsaios" } },
}
```
...
When a file exceeds this limit, BonsaiOS logs a warning and injects a truncated
...
message envelopes). If unset, BonsaiOS uses the host timezone at runtime.
...
`[bonsaios]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[bonsaios]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[bonsaios]"`, BonsaiOS will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, BonsaiOS generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.bonsaios/settings/tts.json",
      elevenlabs: {
...
BonsaiOS also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, BonsaiOS currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `bonsaios configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:bonsaios`: all built-in BonsaiOS tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.bonsaios/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.bonsaios/sandboxes",
        docker: {
          ...
          apparmorProfile: "bonsaios-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "bonsaios-sandbox-browser:bookworm-slim",
          containerPrefix: "bonsaios-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
BonsaiOS uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.bonsaios/agents/<agentId>/agent/models.json` or by defining the same schema inside your
BonsaiOS config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, BonsaiOS writes/merges a `models.json` into
`~/.bonsaios/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. BonsaiOS uses
...
- Shortcut: `bonsaios onboard --auth-choice opencode-zen`.
...
Shortcut: `bonsaios onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `bonsaios onboard --auth-choice moonshot-api-key`.
...
- Set `KIMI_API_KEY` in the environment or use `bonsaios onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `bonsaios onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `bonsaios onboard --auth-choice minimax-api`.
...
- Override the agent config root with `BONSAIOS_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.bonsaios/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.bonsaios/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.bonsaios/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, BonsaiOS stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.bonsaios/skills` (workspace skills
...
from `~/.bonsaios/extensions`, `<workspace>/.bonsaios/extensions`, plus any
...
### `browser` (bonsaios-managed browser)

BonsaiOS can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for bonsaios and expose a small loopback control service.
...
- profile color: `#FF4500` (bonsai-orange)
- Note: the control server is started by the running gateway (BonsaiOS.app menubar, or `bonsaios gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      bonsaios: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/bonsaios" }
...
- Examples: `"/ui"`, `"/bonsaios"`, `"/apps/bonsaios"`.
...
- When a connection comes from one of these IPs, BonsaiOS uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `bonsaios gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `BONSAIOS_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `BONSAIOS_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. BonsaiOS
...
- BonsaiOS.app watches `~/.bonsaios/bonsaios.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.bonsaios/bonsaios.json` (or `BONSAIOS_CONFIG_PATH`) and applies changes automatically.
...
- `~/.bonsaios/bonsaios.json` (or `BONSAIOS_CONFIG_PATH`)
...
- `BONSAIOS_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `bonsaios --dev …` → uses `~/.bonsaios-dev` + shifts ports from base `19001`
- `bonsaios --profile <name> …` → uses `~/.bonsaios-<name>` (port via config/env/flags)
...
Gmail helper config (used by `bonsaios webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "bonsaios@gmail.com",
...
- Set `BONSAIOS_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, BonsaiOS defaults `serve.path` to `/` so
...
Default root: `~/.bonsaios/workspace/canvas`  
Default port: `18793` (chosen to avoid the bonsaios browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__bonsaios__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__bonsaios__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
{
  canvasHost: {
    root: "~/.bonsaios/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
- env: `BONSAIOS_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.bonsaios/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.bonsaios/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.bonsaios/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_bonsaios-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `bonsaios` (advertises `bonsaios.local`). Override with `BONSAIOS_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_bonsaios-gw._tcp` under `~/.bonsaios/dns/` using the configured discovery domain (example: `bonsaios.internal.`).
