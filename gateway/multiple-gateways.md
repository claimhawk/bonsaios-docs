---
summary: "Run multiple BonsaiOS Gateways on one host (isolation, ports, and profiles)"
read_when:
...
- `BONSAIOS_CONFIG_PATH` — per-instance config file
- `BONSAIOS_STATE_DIR` — per-instance sessions, creds, caches
...
Profiles auto-scope `BONSAIOS_STATE_DIR` + `BONSAIOS_CONFIG_PATH` and suffix service names.
