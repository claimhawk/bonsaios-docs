---
summary: "Quick troubleshooting guide for common BonsaiOS failures"
read_when:
...
When BonsaiOS misbehaves, here's how to fix it.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `bonsaios status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `bonsaios status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `bonsaios status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `bonsaios gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `bonsaios channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `bonsaios gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `bonsaios logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `bonsaios status --all` (it redacts tokens). If you paste `bonsaios status`, consider setting `BONSAIOS_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  bonsaios models auth setup-token --provider anthropic
  ```
...
```bash
bonsaios models auth paste-token --provider anthropic
bonsaios models status
```
...
- Preferred: `bonsaios logs --follow`
- File logs (always): `/tmp/bonsaios/bonsaios-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$BONSAIOS_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u bonsaios-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  bonsaios config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  bonsaios config set gateway.mode remote
  bonsaios config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `bonsaios setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.bonsaios/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `bonsaios doctor`
...
- then run `bonsaios sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `bonsaios gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `bonsaios gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `bonsaios gateway status` and `bonsaios doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `BONSAIOS_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `bonsaios gateway status` shows a config mismatch**
...
- Fix: rerun `bonsaios gateway install --force` from the same `--profile` / `BONSAIOS_STATE_DIR` you want the service to use.

**If `bonsaios gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `bonsaios doctor` to update it (or `bonsaios gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `BONSAIOS_GATEWAY_TOKEN`) and restart the service.

**If `bonsaios gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/bonsaios` on disk.
...
Symptoms: `pwd` or file tools show `~/.bonsaios/sandboxes/...` even though you
...
BonsaiOS intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `bonsaios models list` or
  `bonsaios models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${BONSAIOS_CONFIG_PATH:-$HOME/.bonsaios/bonsaios.json}"
```
...
tail -f "$(ls -t /tmp/bonsaios/bonsaios-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
bonsaios logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.bonsaios/agents/<agentId>/sessions/
```
...
bonsaios logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
bonsaios gateway --verbose
```

If you’re logged out / unlinked:

```bash
bonsaios channels logout
trash "${BONSAIOS_STATE_DIR:-$HOME/.bonsaios}/credentials" # if logout can't cleanly remove everything
bonsaios channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/bonsaios/bonsaios-*.log | head -1)" | tail -20
```

### High Memory Usage

BonsaiOS keeps conversation history in memory.
...
BonsaiOS now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
- `bonsaios doctor` reports every invalid entry.
- `bonsaios doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `bonsaios logs`, `bonsaios health`, `bonsaios status`, `bonsaios gateway status`, and `bonsaios gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/bonsaios/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `bonsaios models status` (CLI).
...
5. Run `bonsaios channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. BonsaiOS scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/claimhawk/bonsaios-docs/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.bonsaios.* still works)
```
...
Ensure the global `bonsaios` CLI is installed and matches the app version:

```bash
bonsaios --version
npm install -g bonsaios@<version>
```
...
#   ${BONSAIOS_CONFIG_PATH:-$HOME/.bonsaios/bonsaios.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
bonsaios gateway --verbose
bonsaios channels login --verbose
```
...
| Log                               | Location                                                                                                                                                                                                                                                                                                                    |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway file logs (structured)    | `/tmp/bonsaios/bonsaios-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                                 |
| Gateway service logs (supervisor) | macOS: `$BONSAIOS_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.bonsaios/logs/...`; profiles use `~/.bonsaios-<profile>/logs/...`)<br />Linux: `journalctl --user -u bonsaios-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "BonsaiOS Gateway (<profile>)" /V /FO LIST` |
| Session files                     | `$BONSAIOS_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                            |
| Media cache                       | `$BONSAIOS_STATE_DIR/media/`                                                                                                                                                                                                                                                                                                |
| Credentials                       | `$BONSAIOS_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                          |
...
tail -20 /tmp/bonsaios/bonsaios-*.log
```
...
trash "${BONSAIOS_STATE_DIR:-$HOME/.bonsaios}"
bonsaios channels login         # re-pair WhatsApp
bonsaios gateway restart           # or: bonsaios gateway
```
...
1. Check logs first: `/tmp/bonsaios/` (default: `bonsaios-YYYY-MM-DD.log`, or your configured `logging.file`)
