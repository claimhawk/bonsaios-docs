...
- Replaces the legacy `gateway` command. CLI entry point: `bonsaios gateway`.
...
bonsaios gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
bonsaios gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.bonsaios/bonsaios.json` (or `BONSAIOS_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__bonsaios__/canvas/` from `~/.bonsaios/workspace/canvas`. Disable with `canvasHost.enabled=false` or `BONSAIOS_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `BONSAIOS_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `BONSAIOS_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.bonsaios.*` may still exist)
- Linux: `bonsaios-gateway-<profile>.service`
- Windows: `BonsaiOS Gateway (<profile>)`

Install metadata is embedded in the service config:

- `BONSAIOS_SERVICE_MARKER=bonsaios`
- `BONSAIOS_SERVICE_KIND=gateway`
- `BONSAIOS_SERVICE_VERSION=<version>`
...
```bash
bonsaios --dev setup
bonsaios --dev gateway --allow-unconfigured
...
- `BONSAIOS_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.bonsaios/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `BONSAIOS_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `BONSAIOS_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `BONSAIOS_CONFIG_PATH`
- unique `BONSAIOS_STATE_DIR`
...
```bash
BONSAIOS_CONFIG_PATH=~/.bonsaios/a.json BONSAIOS_STATE_DIR=~/.bonsaios-a bonsaios gateway --port 19001
BONSAIOS_CONFIG_PATH=~/.bonsaios/b.json BONSAIOS_STATE_DIR=~/.bonsaios-b bonsaios gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `bonsaios health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `bonsaios`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `bonsaios gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.bonsaios.*` is cleaned up).
  - `bonsaios doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are BonsaiOS profile services.
  ...
  - Cleanup: `bonsaios gateway uninstall` (current service) and `bonsaios doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `bonsaios gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- BonsaiOS.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.bonsaios.*` labels still unload cleanly).
- To stop it cleanly, use `bonsaios gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `bonsaios gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `bonsaios gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

BonsaiOS installs a **systemd user service** by default on Linux/WSL2. We
...
`bonsaios gateway install` writes the user unit. `bonsaios doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/bonsaios-gateway[-<profile>].service`:

```
[Unit]
Description=BonsaiOS Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/bonsaios gateway --port 18789
Restart=always
RestartSec=5
Environment=BONSAIOS_GATEWAY_TOKEN=
...
```
systemctl --user enable --now bonsaios-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/bonsaios-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now bonsaios-gateway[-<profile>].service
```
...
- `bonsaios gateway health|status` — request health/status over the Gateway WS.
- `bonsaios message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `bonsaios agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `bonsaios gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `bonsaios gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `bonsaios gateway` and the legacy TCP control port.
