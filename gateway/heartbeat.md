...
- During heartbeat runs, BonsaiOS treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `bonsaios sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, BonsaiOS skips the heartbeat run entirely (no model call).
