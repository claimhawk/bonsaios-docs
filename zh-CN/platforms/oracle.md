---
read_when:
  - 在 Oracle Cloud 上部署 BonsaiOS
  - 寻找低成本 VPS 托管方案来运行 BonsaiOS
  - 希望在小型服务器上全天候运行 BonsaiOS
summary: 在 Oracle Cloud（Always Free ARM）上运行 BonsaiOS
title: Oracle Cloud
x-i18n:
...
# 在 Oracle Cloud (OCI) 上运行 BonsaiOS

## 目标

在 Oracle Cloud 的 **Always Free** ARM 层级上运行持久化的 BonsaiOS Gateway网关。

Oracle 的免费层级非常适合运行 BonsaiOS（特别是如果你已有 OCI 账户），但也存在一些权衡：
...
这将启用 Tailscale SSH，让你可以从 tailnet 上的任何设备通过 `ssh bonsaios` 连接，无需公共 IP。
...
**从现在开始，通过 Tailscale 连接：** `ssh ubuntu@bonsaios`（或使用 Tailscale IP）。
...
这种配置通常不再*需要*额外的主机防火墙规则来阻止全网 SSH 暴力破解——但你仍应保持操作系统更新、运行 `bonsaios security audit`，并确认没有意外监听公共接口。
...
- **凭据权限：** `chmod 700 ~/.bonsaios`
- **安全审计：** `bonsaios security audit`
...
- `~/.bonsaios/` — 配置、凭据、会话数据
- `~/.bonsaios/workspace/` — 工作区（SOUL.md、记忆、产物）
