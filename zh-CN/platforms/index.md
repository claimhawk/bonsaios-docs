...
BonsaiOS 核心使用 TypeScript 编写。**推荐使用 Node 作为运行时**。
...
- 服务状态：`bonsaios gateway status`

## Gateway网关服务安装（CLI）

使用以下任一方式（均受支持）：

- 向导（推荐）：`bonsaios onboard --install-daemon`
- 直接安装：`bonsaios gateway install`
- 配置流程：`bonsaios configure` → 选择 **Gateway网关 service**
- 修复/迁移：`bonsaios doctor`（会提示安装或修复服务）

服务目标取决于操作系统：

- macOS：LaunchAgent（`bot.molt.gateway` 或 `bot.molt.<profile>`；旧版 `com.bonsaios.*`）
- Linux/WSL2：systemd 用户服务（`bonsaios-gateway[-<profile>].service`）
