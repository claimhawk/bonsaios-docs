---
read_when:
  - 在 DigitalOcean 上设置 BonsaiOS
  - 寻找便宜的 VPS 托管来运行 BonsaiOS
summary: 在 DigitalOcean 上运行 BonsaiOS（简单的付费 VPS 方案）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 BonsaiOS

## 目标

在 DigitalOcean 上运行持久化的 BonsaiOS Gateway网关，费用为**每月 $6**（预留定价为每月 $4）。
...
- `~/.bonsaios/` — 配置、凭据、会话数据
- `~/.bonsaios/workspace/` — 工作区（SOUL.md、记忆等）
