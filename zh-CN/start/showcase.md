...
summary: 由 BonsaiOS 驱动的社区项目和集成展示
title: 项目展示
x-i18n:
...
来自社区的真实项目。看看大家正在用 BonsaiOS 构建什么。

<Info>
**想要展示你的项目？** 在 [Discord 的 #showcase 频道](https://discord.gg/bonsaios) 分享你的项目，或在 [X 上 @bonsaios](https://x.com/bonsaios)。
</Info>
...
OpenCode 完成修改 → 提交 PR → BonsaiOS 审查差异并在 Telegram 中回复"小建议"以及明确的合并意见（包括需要优先应用的关键修复）。

  <img src="/assets/showcase/pr-review-telegram.jpg" alt="BonsaiOS PR 审查反馈通过 Telegram 发送" />
</Card>

<Card title="几分钟内创建酒窖 Skills" icon="wine-glass" href="https://x.com/i/status/2010916352454791216">
  **@prades_maxime** • `skills` `local` `csv`

让"Robby"（@bonsaios）创建一个本地酒窖 Skills。它请求一个 CSV 导出样本和存储位置，然后快速构建并测试 Skills（示例中有 962 瓶酒）。

  <img src="/assets/showcase/wine-cellar-skill.jpg" alt="BonsaiOS 从 CSV 构建本地酒窖 Skills" />
</Card>
...
桌面应用，用于跨 Agents、Claude、Codex 和 BonsaiOS 管理 Skills/命令。
...
<Card title="Bambu 3D 打印机控制" icon="print" href="https://bonsaihub.com/tobiasbischoff/bambu-cli">
...
<Card title="维也纳公共交通（Wiener Linien）" icon="train" href="https://bonsaihub.com/hjanuschka/wienerlinien">
...
<Card title="R2 上传（发送我的文件）" icon="cloud-arrow-up" href="https://bonsaihub.com/skills/r2-upload">
  **@julianengel** • `files` `r2` `presigned-urls`

上传到 Cloudflare R2/S3 并生成安全的预签名下载链接。非常适合远程 BonsaiOS 实例。
</Card>
...
集成智能体工作流（Claude Code、BonsaiOS）的 Linear CLI。从终端管理 issue、项目和工作流。首个外部 PR 已合并！
</Card>
...
Claude Code 发现并确认了净化器控制方式，然后 BonsaiOS 接管并管理室内空气质量。

  <img src="/assets/showcase/winix-air-purifier.jpg" alt="通过 BonsaiOS 控制 Winix 空气净化器" />
</Card>

<Card title="美丽天空相机抓拍" icon="camera" href="https://x.com/signalgaining/status/2010523120604746151">
  **@signalgaining** • `automation` `camera` `skill` `images`

由屋顶摄像头触发：当天空看起来很美时，让 BonsaiOS 拍一张天空照片——它设计了一个 Skills 并完成了拍摄。

  <img src="/assets/showcase/roof-camera-sky.jpg" alt="BonsaiOS 捕获的屋顶摄像头天空快照" />
</Card>

<Card title="可视化早间简报场景" icon="robot" href="https://x.com/buddyhadry/status/2010005331925954739">
  **@buddyhadry** • `automation` `briefing` `images` `telegram`

定时提示每天早上通过 BonsaiOS 角色生成一张"场景"图片（天气、任务、日期、喜欢的帖子/名言）。
</Card>
...
BonsaiOS 连接到 Jira，然后即时生成了一个新 Skills（在 ClawHub 上还不存在时）。
</Card>
...
自动化 Todoist 任务，并让 BonsaiOS 直接在 Telegram 聊天中生成 Skills。
</Card>
...
<Card title="xuezh 中文学习" icon="language" href="https://github.com/joshp123/xuezh">
  **@joshp123** • `learning` `voice` `skill`
  ...
  通过 BonsaiOS 实现的中文学习引擎，具备发音反馈和学习流程。
...
<Card title="Clawdia 电话桥接" icon="phone" href="https://github.com/alejandroOPI/clawdia-bridge">
  **@alejandroOPI** • `voice` `vapi` `bridge`
  ...
  Vapi 语音助手 ↔ BonsaiOS HTTP 桥接。与你的智能体进行近实时电话通话。
</Card>

<Card title="OpenRouter 转录" icon="microphone" href="https://bonsaihub.com/obviyus/openrouter-transcribe">
...
<Card title="Home Assistant 插件" icon="home" href="https://github.com/ngutman/bonsaios-ha-addon">
  **@ngutman** • `homeassistant` `docker` `raspberry-pi`
  
  在 Home Assistant OS 上运行 BonsaiOS Gateway网关，支持 SSH 隧道和持久化状态。
</Card>

<Card title="Home Assistant Skills" icon="toggle-on" href="https://bonsaihub.com/skills/homeassistant">
  **ClawHub** • `homeassistant` `skill` `automation`
  
  通过自然语言控制和自动化 Home Assistant 设备。
</Card>

<Card title="Nix 打包" icon="snowflake" href="https://github.com/claimhawk/nix-bonsaios">
  **@bonsaios** • `nix` `packaging` `deployment`
  
  开箱即用的 Nix 化 BonsaiOS 配置，实现可复现部署。
</Card>

<Card title="CalDAV 日历" icon="calendar" href="https://bonsaihub.com/skills/caldav-calendar">
...
<Card title="GoHome 自动化" icon="house-signal" href="https://github.com/joshp123/gohome">
  **@joshp123** • `home` `nix` `grafana`
  ...
  以 Nix 为基础的家居自动化，BonsaiOS 作为交互界面，并配有精美的 Grafana 仪表盘。
...
<Card title="StarSwap 交易市场" icon="star" href="https://star-swap.com/">
  **社区** • `marketplace` `astronomy` `webapp`
  ...
  完整的天文设备交易市场。基于 BonsaiOS 生态系统构建。
</Card>
...
<Steps>
  <Step title="分享它">
    发布到 [Discord 的 #showcase 频道](https://discord.gg/bonsaios) 或 [在推特上 @bonsaios](https://x.com/bonsaios)
  </Step>
  <Step title="提供详细信息">
