...
通过一键模板在 Railway 上部署 BonsaiOS，并在浏览器中完成设置。
...
- `https://<your-railway-domain>/bonsaios` — 控制面板 UI
...
- `BONSAIOS_STATE_DIR=/data/.bonsaios`（推荐）
- `BONSAIOS_WORKSPACE_DIR=/data/workspace`（推荐）
- `BONSAIOS_GATEWAY_TOKEN`（推荐；请视为管理员密钥）
...
这会导出你的 BonsaiOS 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。
