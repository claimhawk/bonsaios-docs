...
你也可以设置 `BONSAIOS_VERBOSE=1` 来代替该标志。
...
### `docs.bonsaios.com` 显示 SSL 错误（Comcast/Xfinity）

某些 Comcast/Xfinity 连接通过 Xfinity Advanced Security 屏蔽 `docs.bonsaios.com`。
禁用 Advanced Security 或将 `docs.bonsaios.com` 添加到允许列表，然后重试。
...
- 检查允许列表：`bonsaios config get agents.defaults.models`
...
如果可以，请附上 `bonsaios logs --follow` 中相关的日志尾部内容。
