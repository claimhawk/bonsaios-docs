...
read_when:
  - 安装 BonsaiOS
  - 你想从 GitHub 安装
summary: 安装 BonsaiOS（推荐安装器、全局安装或从源码安装）
title: 安装
x-i18n:
...
pnpm approve-builds -g                # 批准 bonsaios、node-llama-cpp、sharp 等
pnpm add -g bonsaios@latest           # 重新运行以执行 postinstall 脚本
```
...
提示：如果尚未全局安装，可通过 `pnpm bonsaios ...` 运行仓库命令。
...
- 运行新手引导：`bonsaios onboard --install-daemon`
- 快速检查：`bonsaios doctor`
- 检查 Gateway网关健康状态：`bonsaios status` + `bonsaios health`
- 打开仪表盘：`bonsaios dashboard`
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不在** `echo "$PATH"` 的输出中，说明你的 shell 无法找到全局 npm 二进制文件（包括 `bonsaios`）。
