...
read_when:
  ...
  - 你想启用 OpenProse 插件
  - 你需要了解状态存储
summary: OpenProse：BonsaiOS 中的 .prose 工作流、斜杠命令和状态管理
title: OpenProse
x-i18n:
...
OpenProse 是一种可移植的、以 Markdown 为核心的工作流格式，用于编排 AI 会话。在 BonsaiOS 中，它以插件形式提供，安装后包含一个 OpenProse Skills 包和一个 `/prose` 斜杠命令。程序存放在 `.prose` 文件中，可以通过显式控制流生成多个子智能体。
...
开发/本地签出：`bonsaios plugins install ./extensions/open-prose`

相关文档：[插件](/plugin)、[插件清单](/plugins/manifest)、[Skills](/tools/skills)。

## 斜杠命令

OpenProse 注册 `/prose` 作为用户可调用的 Skills 命令。它路由到 OpenProse 虚拟机指令，底层使用 BonsaiOS 工具。
...
OpenProse 程序映射到 BonsaiOS 原语：

| OpenProse 概念       | BonsaiOS 工具    |
| -------------------- | ---------------- |
...
请像对待代码一样对待 `.prose` 文件。运行前请先审查。使用 BonsaiOS 工具允许列表和审批门控来控制副作用。

如需确定性的、带审批门控的工作流，可参考 [Bonsai](/tools/bonsai) 进行比较。
