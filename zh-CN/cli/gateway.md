...
summary: BonsaiOS Gateway网关 CLI（`bonsaios gateway`）— 运行、查询和发现 Gateway网关
title: gateway
x-i18n:
...
Gateway网关是 BonsaiOS 的 WebSocket 服务器（渠道、节点、会话、钩子）。

本页中的子命令位于 `bonsaios gateway …` 下。
...
- 默认情况下，除非在 `~/.bonsaios/bonsaios.json` 中设置了 `gateway.mode=local`，否则 Gateway网关会拒绝启动。使用 `--allow-unconfigured` 进行临时/开发运行。
...
- `--token <token>`：令牌覆盖（同时为进程设置 `BONSAIOS_GATEWAY_TOKEN`）。
- `--password <password>`：密码覆盖（同时为进程设置 `BONSAIOS_GATEWAY_PASSWORD`）。
...
`gateway discover` 扫描 Gateway网关信标（`_bonsaios-gw._tcp`）。

- 组播 DNS-SD：`local.`
- 单播 DNS-SD（广域 Bonjour）：选择一个域名（例如：`bonsaios.internal.`）并设置分离 DNS + DNS 服务器；参见 [/gateway/bonjour](/gateway/bonjour)
