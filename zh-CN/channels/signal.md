...
2. 链接机器人账户：
   - `signal-cli link -n "BonsaiOS"` 然后在 Signal 中扫描二维码。
...
如果你想自行管理 `signal-cli`（JVM 冷启动慢、容器初始化或共享 CPU），可以单独运行守护进程并将 BonsaiOS 指向它：
...
这会跳过 BonsaiOS 内部的自动启动和启动等待。当自动启动较慢时，请设置 `channels.signal.startupTimeoutMs`。
...
- **输入指示**：BonsaiOS 通过 `signal-cli sendTyping` 发送输入信号，并在回复运行期间刷新。
- **已读回执**：当 `channels.signal.sendReadReceipts` 为 true 时，BonsaiOS 为允许的私信转发已读回执。
