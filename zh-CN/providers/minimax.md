---
read_when:
  - 你想在 BonsaiOS 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指导
summary: 在 BonsaiOS 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
bonsaios plugins enable minimax-portal-auth  # 如已加载则跳过
bonsaios gateway restart  # 如 Gateway网关已在运行则重启
...
详情请参阅 [MiniMax OAuth 插件 README](https://github.com/claimhawk/bonsaios-docs/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `bonsaios models list` 和 `bonsaios models set minimax/MiniMax-M2.1` 进行切换。
...
- 运行 `bonsaios configure` 并选择 **MiniMax M2.1**，或
