...
This is handy when you need to toggle obscure settings without editing `bonsaios.json`.
...
- **Global `--dev` (profile):** isolates state under `~/.bonsaios-dev` and
...
If you don’t have a global install yet, run the CLI via `pnpm bonsaios ...`.
...
1. **Profile isolation** (global `--dev`)
   ...
   - `BONSAIOS_GATEWAY_PORT=19001` (browser/canvas shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`BONSAIOS_SKIP_CHANNELS=1`).
...
## Raw stream logging (BonsaiOS)

BonsaiOS can log the **raw assistant stream** before any filtering/formatting.
