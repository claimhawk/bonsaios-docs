...
- Jobs persist under `~/.bonsaios/cron/` so restarts don’t lose schedules.
...
bonsaios cron list
bonsaios cron run <job-id> --force
bonsaios cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.bonsaios/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `bonsaios cron add/edit` or the cron
...
- Job store: `~/.bonsaios/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.bonsaios/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
bonsaios cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
bonsaios cron edit <jobId> --agent ops
bonsaios cron edit <jobId> --clear-agent
```
...
```bash
bonsaios system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`bonsaios system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `BONSAIOS_SKIP_CRON`.
