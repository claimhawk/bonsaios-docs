...
Deploy BonsaiOS on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a
  href="https://render.com/deploy?repo=https://github.com/claimhawk/bonsaios-docs"
  target="_blank"
  rel="noreferrer"
>
  Deploy to Render
...
repository configures everything needed to run BonsaiOS:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: BONSAIOS_STATE_DIR
        value: /data/.bonsaios
      - key: BONSAIOS_WORKSPACE_DIR
        value: /data/workspace
      - key: BONSAIOS_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/bonsaios`.
...
If you use the original BonsaiOS repository, Render will not auto-deploy your BonsaiOS. To update it, run a manual Blueprint sync from the dashboard.
...
For BonsaiOS, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any BonsaiOS host.
