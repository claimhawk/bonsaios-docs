...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/bonsaios`)
...
If the page fails to load, start the Gateway first: `bonsaios gateway`.
...
you revoke it with `bonsaios devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.bonsaios/bonsaios.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. BonsaiOS
...
```bash
bonsaios gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
BonsaiOS **blocks** Control UI connections without device identity.
...
```bash
BONSAIOS_CONTROL_UI_BASE_PATH=/bonsaios/ pnpm ui:build
```
