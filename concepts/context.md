...
read_when:
  - You want to understand what “context” means in BonsaiOS
...
“Context” is **everything BonsaiOS sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (BonsaiOS-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How BonsaiOS builds the system prompt

The system prompt is **BonsaiOS-owned** and rebuilt each run. It includes:
...
By default, BonsaiOS injects a fixed set of workspace files (if present):
