...
- `/usage cost` in chats: local cost summary aggregated from BonsaiOS session logs.
- CLI: `bonsaios status --usage` prints a full per-provider breakdown.
- CLI: `bonsaios channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
