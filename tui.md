...
```bash
bonsaios tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `bonsaios tui --deliver`
...
- Check the Gateway logs: `bonsaios logs --follow`.
- Confirm the agent can run: `bonsaios status` and `bonsaios models status`.
...
- No agents in picker: check `bonsaios agents list` and your routing config.
