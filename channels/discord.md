...
- Talk to BonsaiOS via Discord DMs or guild channels.
...
3. Configure BonsaiOS with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `bonsaios pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running BonsaiOS in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents BonsaiOS needs
...
Discord uses numeric ids everywhere; BonsaiOS config prefers ids.
...
- First: run `bonsaios doctor` and `bonsaios channels status --probe` (actionable warnings + quick audits).
...
When enabled, BonsaiOS uses the member identity for allowlists and labels the
...
- On startup, BonsaiOS resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:

- The registered commands mirror BonsaiOS’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; BonsaiOS enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`bonsaios gateway --force`) after confirming no other processes own the Discord session.
