...
3. Configure BonsaiOS with `channels.imessage.cliPath` and `channels.imessage.dbPath`.
...
- Full Disk Access for BonsaiOS + `imsg` (Messages DB access).
...
2. Create a macOS user (example: `bonsaioshome`) and sign into it.
...
If you want iMessage on another Mac, set `channels.imessage.cliPath` to a wrapper that runs `imsg` on the remote macOS host over SSH. BonsaiOS only needs stdio.
...
**Remote attachments:** When `cliPath` points to a remote host via SSH, attachment paths in the Messages database reference files on the remote machine. BonsaiOS can automatically fetch these over SCP by setting `channels.imessage.remoteHost`:
...
If `remoteHost` is not set, BonsaiOS attempts to auto-detect it by parsing the SSH command in your wrapper script. Explicit configuration is recommended for reliability.
...
│ - bonsaios gateway           │          SCP (attachments)        │ - Messages signed in     │
...
```json5
{
  channels: {
    imessage: {
      enabled: true,
      cliPath: "~/.bonsaios/scripts/imsg-ssh",
...
Example wrapper (`~/.bonsaios/scripts/imsg-ssh`):
...
- Approve via:
  - `bonsaios pairing list imessage`
  - `bonsaios pairing approve imessage <CODE>`
...
If you explicitly configure a `chat_id` under `channels.imessage.groups`, BonsaiOS treats that thread as a “group” for:
