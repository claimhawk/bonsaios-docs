...
```bash
bonsaios plugins install @bonsaios/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
bonsaios plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "BonsaiOS" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `bonsaios pairing list nextcloud-talk`
  - `bonsaios pairing approve nextcloud-talk <CODE>`
