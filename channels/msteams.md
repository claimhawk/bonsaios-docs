...
```bash
bonsaios plugins install @bonsaios/msteams
```

Local checkout (when running from a git repo):

```bash
bonsaios plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
BonsaiOS will offer the local install path automatically.
...
3. Configure BonsaiOS with those credentials.
...
- Talk to BonsaiOS via Teams DMs, group chats, or channels.
...
- On startup, BonsaiOS resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.bonsaios/bonsaios.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring BonsaiOS, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `bonsaios-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `bonsaios plugins install @bonsaios/msteams`
   - From a local checkout: `bonsaios plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "BonsaiOS in Teams", "full": "BonsaiOS in Teams" },
...
BonsaiOS handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, BonsaiOS only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/BonsaiOSShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

BonsaiOS sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `bonsaios message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.bonsaios/msteams-polls.json`.
...
```bash
bonsaios message send --channel msteams \
...
bonsaios message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
bonsaios message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
bonsaios message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
bonsaios message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
