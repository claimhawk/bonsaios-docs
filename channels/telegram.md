...
BonsaiOS can stream partial replies in Telegram DMs using `sendMessageDraft`.
...
- If Telegram rejects the HTML payload, BonsaiOS retries the same message as plain text.

## Commands (native + custom)

BonsaiOS registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
- Custom commands are **menu entries only**; BonsaiOS does not implement them unless you handle them elsewhere.
...
**Privacy note:** `@userinfobot` is a third-party bot. If you prefer, add the bot to the group, send a message, and use `bonsaios logs --follow` to read `chat.id`, or use the Bot API `getUpdates`.
...
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). BonsaiOS can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. BonsaiOS:
...
Private chats can include `message_thread_id` in some edge cases. BonsaiOS keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
2. Run `bonsaios logs --follow` and look for `from.id`.
...
BonsaiOS defaults to audio files for backward compatibility.
...
BonsaiOS supports receiving and sending Telegram stickers with intelligent caching.

### Receiving stickers

When a user sends a sticker, BonsaiOS handles it based on the sticker type:
...
Stickers are processed through the AI's vision capabilities to generate descriptions. Since the same stickers are often sent repeatedly, BonsaiOS caches these descriptions to avoid redundant API calls.
...
**Cache location:** `~/.bonsaios/telegram/sticker-cache.json`
...
BonsaiOS uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, BonsaiOS:
...
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by BonsaiOS)
...
- Example: `bonsaios message send --channel telegram --target 123456789 --message "hi"`.
...
- `bonsaios channels status` shows a warning when config expects unmentioned group messages.
- `bonsaios channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `bonsaios logs --follow` (look for "skipping group message")
...
- Upgrade to a BonsaiOS build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
