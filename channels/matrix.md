...
Matrix is an open, decentralized messaging protocol. BonsaiOS connects as a Matrix **user**
...
1. Install the Matrix plugin:
   - From npm: `bonsaios plugins install @bonsaios/matrix`
   - From a local checkout: `bonsaios plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: BonsaiOS calls the same
     login endpoint, stores the access token in `~/.bonsaios/credentials/matrix/credentials.json`,
...
- On first connection, BonsaiOS requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  BonsaiOS logs a warning.
...
`~/.bonsaios/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, BonsaiOS resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
