...
Tlon is a decentralized messenger built on Urbit. BonsaiOS connects to your Urbit ship and can
...
```bash
bonsaios plugins install @bonsaios/tlon
```

Local checkout (when running from a git repo):

```bash
bonsaios plugins install ./extensions/tlon
```
...
Use these with `bonsaios message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, BonsaiOS replies in-thread.
