...
read_when:
  - Setting up Twitch chat integration for BonsaiOS
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. BonsaiOS connects as a Twitch user (bot account) to receive and send messages in channels.
...
4. Configure the token:
   - Env: `BONSAIOS_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "bonsaios", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use BONSAIOS_TWITCH_ACCESS_TOKEN env var)
