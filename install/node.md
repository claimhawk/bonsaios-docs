...
read_when:
  - "You installed BonsaiOS but `bonsaios` is “command not found”"
...
BonsaiOS’s runtime baseline is **Node 22+**.

If you can run `npm install -g bonsaios@latest` but later see `bonsaios: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
