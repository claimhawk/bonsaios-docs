...
BonsaiOS features that can generate provider usage or paid API calls.
...
- `bonsaios status --usage` and `bonsaios channels list` show provider **usage windows**
...
BonsaiOS can pick up credentials from:
...
- `bonsaios status --usage`
- `bonsaios models status --json`
...
`bonsaios models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
