...
You can also set `BONSAIOS_VERBOSE=1` instead of the flag.
...
### `docs.bonsaios.com` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.bonsaios.com` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.bonsaios.com` to the allowlist, then retry.
...
- Check the allowlist: `bonsaios config get agents.defaults.models`
...
If you can, include the relevant log tail from `bonsaios logs --follow`.
