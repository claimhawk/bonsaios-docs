...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `bonsaios sandbox explain`.
...
Legacy `agent.*` configs are migrated by `bonsaios doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=bonsaios-sbx-"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${BONSAIOS_STATE_DIR:-$HOME/.bonsaios}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
