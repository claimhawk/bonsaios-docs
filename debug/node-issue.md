...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in BonsaiOS
...
Running BonsaiOS via Node with `tsx` fails at startup with:

```
[bonsaios] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `bonsaios status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch bonsaios.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node bonsaios.mjs status` works on Node 25.
