---
summary: "Agent tool surface for BonsaiOS (browser, canvas, nodes, message, cron) replacing legacy `bonsaios-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `bonsaios-*` skills
title: "Tools"
---

# Tools (BonsaiOS)

BonsaiOS exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `bonsaios-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `bonsaios.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, BonsaiOS logs a warning and ignores the allowlist so core tools stay available.
...
- [Bonsai](/tools/bonsai): typed workflow runtime with resumable approvals (requires the Bonsai CLI on the gateway host).
...
- `host=node` can target a macOS companion app or a headless node host (`bonsaios node run`).
...
- Requires a Brave API key (recommended: `bonsaios configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated BonsaiOS-managed browser.
...
- Quick smoke: `bonsaios nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `bonsaios gateway` restart in-place)
