---
summary: "Chrome extension: let BonsaiOS drive your existing Chrome tab"
read_when:
...
The BonsaiOS Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate bonsaios-managed Chrome profile.
...
BonsaiOS then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the BonsaiOS release (npm package) as static files. There is no separate “build” step.

After upgrading BonsaiOS:

- Re-run `bonsaios browser extension install` to refresh the installed files under your BonsaiOS state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

BonsaiOS ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want BonsaiOS to control.
...
- `ON`: attached; BonsaiOS can drive that tab.
...
Debugging: `bonsaios sandbox explain`
...
`bonsaios browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `bonsaios browser extension install` first to copy the extension to a stable location under your BonsaiOS state directory.
...
- **This is not isolated** like the dedicated bonsaios-managed profile.
