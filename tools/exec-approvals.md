...
- **gateway host** → `bonsaios` process on the gateway machine
...
`~/.bonsaios/exec-approvals.json` directly.

CLI: `bonsaios approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
