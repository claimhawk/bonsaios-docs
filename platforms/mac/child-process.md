...
service via the external `bonsaios` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`BONSAIOS_PROFILE`; legacy `com.bonsaios.*` is supported).
...
- Writes `~/.bonsaios/disable-launchagent`.
...
`--attach-only` (or `--no-launchd`). This sets `~/.bonsaios/disable-launchagent`,
