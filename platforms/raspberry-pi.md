---
summary: "BonsaiOS on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up BonsaiOS on a Raspberry Pi
  - Running BonsaiOS on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# BonsaiOS on Raspberry Pi

## Goal

Run a persistent, always-on BonsaiOS Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most BonsaiOS features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled bonsaios

# Enable if not
sudo systemctl enable bonsaios

# Start on boot
sudo systemctl start bonsaios
```
...
```bash
# Check logs
journalctl -u bonsaios --no-pager -n 100

# Common fix: rebuild
cd ~/bonsaios  # if using hackable install
npm run build
sudo systemctl restart bonsaios
```
