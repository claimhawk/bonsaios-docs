...
read_when:
  - Designing workspace memory (~/.bonsaios/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep BonsaiOS integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.bonsaios/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.bonsaios/workspace` as canonical human-readable memory.
...
```
~/.bonsaios/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in BonsaiOS**, but keep a separable core library.

### Why integrate into BonsaiOS?

- BonsaiOS already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `bonsaios memory recall "…" --k 25 --since 30d`
  - `bonsaios memory reflect --since 7d`
...
Pragmatic take for `~/.bonsaios/workspace`:
