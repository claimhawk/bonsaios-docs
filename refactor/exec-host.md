...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.bonsaios/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.bonsaios/node.json` (node id + pairing token).
...
Path: `~/.bonsaios/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.bonsaios/exec-approvals.sock",
...
- Unix socket at `~/.bonsaios/exec-approvals.sock` (0600).
