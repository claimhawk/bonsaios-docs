...
- Publish as `bonsaios/plugin-sdk` (or export from core under `bonsaios/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `BonsaiOSPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: BonsaiOSConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: BonsaiOSConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: BonsaiOSConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: BonsaiOSConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: BonsaiOSConfig): string;
  };
};
```
...
- Introduce `bonsaios/plugin-sdk`.
- Add `api.runtime` to `BonsaiOSPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `bonsaiosRuntime: ">=2026.2.0"`).
